...
export function get_NickMapControls(map:OpenLayersMap, host:IVisualHost){
    let react_root = document.createElement("div")
    ReactDOM.render(
        <NickMapControlsComponent
            map={map}
            host={host}
            status_text="status"
    ...
    return new Control({ element: react_root });
}
...
function NickMapControlsComponent (props:NickMapControlsComponentProps){
    let [hidden, set_hidden] = React.useState(false)
    return <div id="nickmap-controls-container">
        <div id="nickmap-controls-container-title">
            <button onClick={()=>set_hidden(!hidden)}>{hidden? "show" : "hide"}</button>
            <div>Controls</div>
        </div>
        <div id="nickmap-controls-collapsible-body" className={hidden?'':'expanded'}>
            <div id="nickmap-controls-other-map-button-container">
                <button
                    id="nickmap-controls-open-in-google-button"
                    title="Open current view in google maps"
                    onClick={()=>goto_google_maps(props.map,props.host)}
                ></button>
                <div
            ...
            <div id="nickmap-controls-status-text">{props.status_text}</div>
            <div id="nickmap-controls-version-display">{props.version_display}</div>
        </div>
    </div>;
}
//     get control(){
